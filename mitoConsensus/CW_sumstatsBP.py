...
def main(out_dir, sample, barcode_tag, base_qual, pcr_qual):
    ...
    ## Loop through each molecule for genotyping.  The summary of fragment level and pcr level are conducted
    Frag_TB=0  #Summarize total bases at fragment level
    Frag_OB=0  #Summarize overlapped bases at fragment level
    Frag_rm_LQ=0 #Summarize qualified bases after removing strand conflict and low Q bases, which is out of OB at fragment level
    Frag_rm_PE=0 # Summarize qualified bases after removing PCR error bases, which is out of LQ at fragment level

    PCR_TB=0  #Summarize total bases at pcr level
    PCR_OB=0  #Summarize overlapped bases at pcr level
    PCR_LQ=0 #Summarize number of bases with low quality in either strand
    PCR_SC=0 #Summarize number of bases with strand conflict at pcr level

    MolecularN=len(MoleculeDict)
    bar = Bar('Genotyping', max=MolecularN)
    for index, m in enumerate(MoleculeDict):
        CellBC=m.split('_')[0]
        FragmentSize=abs(int(m.split('_')[2])-int(m.split('_')[1]))
        Frag_TB=Frag_TB+FragmentSize
        PCR_TB=PCR_TB+FragmentSize*len(MoleculeDict[m])
        ...
        for read_pair in MoleculeDict[m]:
            ...
            for base_0,base_1 in zip(pos_array_overlap_0,pos_array_overlap_1):
                overlap_bases+=1
                if(quality_0[base_0[0]]>base_qual or quality_1[base_1[0]]>base_qual):
        ...
        ##Before consider PCR errors, summarize the bases(at fragment level) left after remove the strand bias
        Frag_OB=Frag_OB+np.amax(overlap)
        PCR_OB=PCR_OB+overlap_bases
        PCR_LQ=PCR_LQ+LowQ_bases
        PCR_SC=PCR_SC+Conflict_strand_bases
        Frag_rm_LQ=Frag_rm_LQ+MoleculeGenotypes[np.sum(MoleculeGenotypes,axis=1)>0].shape[0]
        ...
        Conflict_pcr_bases=np.count_nonzero(np.logical_and(np.amax(Molecule_con,axis=1)>0,np.amax(Molecule_con,axis=1)<pcr_qual))  ## This number is at molecule(fragment) level
        Molecule_con[Molecule_con<pcr_qual]=0
        Molecule_con[Molecule_con>pcr_qual]=1
        Frag_rm_PE=Frag_rm_PE+Molecule_con[np.sum(Molecule_con,axis=1)>0].shape[0]
    ...
    out_file_qc_summary=out_pre + "."+"QC.summary"
    ...
    # Write the final QC summary
    FinalQC=pd.DataFrame({"N":[Frag_TB,Frag_OB,Frag_rm_LQ,Frag_rm_PE,PCR_TB,PCR_OB,PCR_LQ,PCR_SC],"Name":["Frag_TB","Frag_OB","Frag_rm_LQ","Frag_rm_PE","PCR_TB","PCR_OB","PCR_LQ","PCR_SC"],"cat":["Frag","Frag","Frag","Frag","PCR","PCR","PCR","PCR"]})
    FinalQC.to_csv(out_file_qc_summary,index=False)
