> Note: this is the original manual tutorial from 2022. For current multi-sample production runs on the Weissman lab cluster, use the shared Slurm workflow described in [README.md](README.md) and [docs/slurm-workflow.md](docs/slurm-workflow.md).
...
## Assign paths
```console
REDEEM_V=ThePathToREDEEM-V #The loacation where the REDEEM-V is downloaded to (eg. /path/to/REDEEM-V/)
...
bowtie2-build hg38.mitoMask.fa hg38.mitoMask  ## This takes time and will generate following, if the index already exists, symlink or point `GENOME_PREFIX` to that Bowtie2 prefix
```
